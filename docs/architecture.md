...
Its separate default-off completed-view feature enters only through the existing teardown authorization path, replaces the stopped task pane with a bounded static no-agent tab rooted outside the disposable worktree, and then retires the ordinary task from monitoring; [`bin/fm-completed-view-lib.sh`](../bin/fm-completed-view-lib.sh) owns its record and retention contracts, while [Completed task views](herdr-backend.md#completed-task-views) owns operator behavior and safety limits.
...
Firstmate-managed tracked code changes use a second mate as coordinator over separate implementation and read-only review workers, with one PR iterated to a clean complete-diff review before it can be presented for merge authority.
The [`nathan-coding-loop` skill](../.agents/skills/nathan-coding-loop/SKILL.md) is the single owner of the fixed worker profiles, profile preflight, PR-visible finding and response protocol, repeated current-head review, and unchanged merge-authority boundary.
