...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true` and pins `commands.lint` to run `bin/fm-personal-pr-target-check.sh --repository NathanAW24/firstmate --base nathan-main --delivery no-mistakes --gate-worktree` before `bin/fm-lint.sh`, so the personal fork target is verified at the gate's lint step before any push or PR.
