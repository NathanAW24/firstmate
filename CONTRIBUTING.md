...
`.no-mistakes.yaml` publishes test evidence to the orphan `no-mistakes/evidence` branch, which shares no history with code branches; gate defaults including the pinned lint command are owned by [`docs/configuration.md`](docs/configuration.md#gate-defaults-no-mistakesyaml).
